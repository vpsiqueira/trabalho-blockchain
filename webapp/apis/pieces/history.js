...
module.exports = {
  ...
  addHistory: async function(req, res) {
    if (!req.session.username) {
      ...
      console.log("*** Apis -> pieces -> history: ***");
      console.log(req.body);

      let dates = [];
      let stages = [];

      const { pieceId, stage, date } = req.body;
      ...
      try {
        ...
        if (accountUnlocked) {
          await MyContract.methods
            .addNewHistory(pieceId, stages, dates)
            .send({ from: userAddr, gas: 3000000 })
            .then(function(result) {
              console.log(result);
              return res.send({
                error: false,
                msg: "Histórico cadastrado com sucesso."
              });
            })
            .catch(function(err) {
              console.log(err);
  ...
  getHistory: async function(req, res) {
    ...
    await MyContract.methods
      ...
      .then(async function(his) {
        ...
        res.send({
          error: false,
          msg: "Histórico resgatado com sucesso",
          historiesArray
        });
      })
      .catch(error => {
        console.log(
          "*** apis -> pieces -> history -> getHistory ERROR: ***",
          error
        );
