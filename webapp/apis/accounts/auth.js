...
const allAccountsInfo = require("../../utils/parityRequests")
...
async function register(req, res) {
  ...
  try {
    ...
    let NewAccountResponse = await axios.post(httpEndpoint, newAccountRequest, {
      headers: headers
    });
    accountAddress = NewAccountResponse.data.result;
    console.log(
      "Account created " + JSON.stringify(NewAccountResponse.data.result)
    );
    ...
    let setAccountNameRequest = {
      ...
      params: [accountAddress, name],
      id: 1,
      jsonrpc: "2.0"
    };
    let setAccountNameResponse = await axios.post(
      httpEndpoint,
      setAccountNameRequest,
      { headers: headers }
    );
    console.log(
      "Account name setup status: %s",
      JSON.stringify(setAccountNameResponse.data.result)
    );

    // Desbloqueia a conta do usuário para salvar seus dados
    // Ex: email
    let unlockResponse = await web3.eth.personal.unlockAccount(
      accountAddress,
      pass,
      null
    );
    ...
    if (unlockResponse) {
      ...
      sendFundsResponse = await web3.eth.sendTransaction({
      ...
      if (sendFundsResponse) {
        MyContract.methods
          .setUser(accountAddress, "email@gmail.com")
          .send({ from: accountAddress, gas: 3000000 })
          .then(function(result) {
            console.log("*** Usuário registrado ***");
            return res
              .status(200)
              .json({ error: false, msg: "Conta criada com sucesso." });
          })
...
async function login(req, res) {
  ...
  let accounts = [];
  await axios
    .post(httpEndpoint, allAccountsInfo, { headers })
  ...
  await web3.eth.personal
    ...
    .then(function(result) {
      ...
      return res
        .status(200)
        .json({ error: false, userData: { name: user, address: userAddr } });
    })
    .catch(function(error) {
